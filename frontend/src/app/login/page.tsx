...
function LoginForm() {
  ...
  // OAuth コールバック後 (oauth=success) → fetchMe して認証状態を更新
  useEffect(() => {
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label
              htmlFor={usernameId}
              className="block text-sm font-bold mb-2 text-muted"
            >
              {t.username}
            </label>
            <input
        ...
        <p className="text-center mt-4 text-sm text-muted">
          {t.noAccount}{" "}
          <Link href={from ? `/register?from=${encodeURIComponent(safeRedirectTarget(from))}` : "/register"} className="text-accent underline">
            {t.register}
          </Link>
        </p>
      </div>
