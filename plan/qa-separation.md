...
| **お気に入り** | `Bookmark.comment` でコメントをお気に入りにできる | 変更なし。質問はお気に入りにできなくなる |
...
**2026-08-02 に段階1〜5すべて実装し、PR #61 でマージ・本番反映まで完了。**
以下は実装時の区切りの記録。

実装時に分かったこと：
- 分岐元の main が古く、`origin/main` は190ファイル進んでいた（デザイン・アクセシビリティの
  全面改修）。統合で13ファイルが衝突。**着手前に `git fetch` すること。**
- 向こうの改修は残した：タブの `role="tablist"` とキーボード操作、記事取得の失敗表示、
  `useLoadMore` の `error`/`retry`、メール通知（`notifications/services.py`）。
- Q&A の回答通知も `send_user_notification` に通し、受け取る人のメール設定に従うようにした。
...
5. 迷ったら**機能を足さない**。投票・お気に入りは落とす決定をした。後から足す方が安い。
