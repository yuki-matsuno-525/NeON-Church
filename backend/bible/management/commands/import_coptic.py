...
使い方:
  ...
  python manage.py import_coptic /path/to/corpora-master --license open
  python manage.py import_coptic /path/to/corpora-master --skip-en

ライセンスティア:
  open  CC-BY / CC-BY-SA / パブリックドメイン（商用利用可）
  cc    open + CC BY-NC-SA（非商用制限あり）
  all   学術限定（Warren Wells）・不明を含む全て（デフォルト）
...
def _classify_license(license_str: str) -> str:
    """ライセンス文字列を 'open' / 'nc' / 'academic' / 'unknown' に分類する。"""
    s = license_str.lower()
    if "warren wells" in s or "academic use only" in s:
        return "academic"
    if "nc" in s:
        return "nc"
    if any(k in s for k in ("cc-by", "cc by", "public domain", "creative commons")):
        return "open"
    return "unknown"


# --license 引数の値 → 許可するライセンスクラスのセット
LICENSE_TIERS: dict[str, set[str]] = {
    "open": {"open"},
    "cc": {"open", "nc"},
    "all": {"open", "nc", "academic", "unknown"},
}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        license_tier = options["license"]
        allowed_licenses = LICENSE_TIERS[license_tier]
        self.stdout.write(f"ライセンスティア: {license_tier} ({allowed_licenses})")
        ...
        for corpus_dir in ordered_dirs:
            ...
            verses_added = self._import_corpus(
                corpus_dir, conllu_source, meta, include_en, nc_counters, allowed_licenses
            )
    ...
    def _import_corpus(
        ...
        for stem, content in _iter_conllu(conllu_source):
            ...
            # ライセンスチェック
            lic_class = _classify_license(meta_entry.get("license", ""))
            if lic_class not in allowed_licenses:
                skipped_license += 1
                continue
        ...
        if skipped_license:
            self.stdout.write(
                self.style.WARNING(f"  → ライセンス除外: {skipped_license} ファイルをスキップ")
            )
